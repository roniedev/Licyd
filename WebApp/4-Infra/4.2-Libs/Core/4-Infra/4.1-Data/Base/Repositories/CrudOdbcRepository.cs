    ...
    ICrudOdbcRepository<T> where T : IBaseAuditEntity
{
    ...
    public virtual void Activate(long id, OdbcConnection conn, OdbcTransaction transaction)
    {
        (string sql, DynamicParameters param) = _sqlHelper.GetUpdateStatusCommand<T>(id, StatusEnum.Active, true);
    ...
    public virtual void Inactivate(long id, string reasonInactivation, OdbcConnection conn, OdbcTransaction transaction)
    {
        (string sql, DynamicParameters param) = _sqlHelper.GetUpdateStatusCommand<T>(id, StatusEnum.Inactive, true, reasonInactivation);
        conn.Execute(sql, param, transaction);
    }

    public virtual TResult BeginTransaction<TResult>(Func<OdbcConnection, OdbcTransaction, TResult> func)
    {
    ...
    public virtual void BeginTransaction(Action<OdbcConnection, OdbcTransaction> action)
    {
        var conn = new OdbcConnection(_appSettings.Connection.DB2);
        conn.Open();
    ...
    public virtual async Task<TResult> BeginTransactionAsync<TResult>(Func<OdbcConnection, OdbcTransaction, Task<TResult>> func)
    {
        var conn = new OdbcConnection(_appSettings.Connection.DB2);
        conn.Open();
    ...
    public virtual async Task BeginTransactionAsync(Action<OdbcConnection, OdbcTransaction> action)
    {
        var conn = new OdbcConnection(_appSettings.Connection.DB2);
    ...
    public virtual T Insert(T entity)
    {
        ...
        (string sql, DynamicParameters param) = entity is IBaseStatusEntity
            ? _sqlHelper.GetInsertCommand(entity, true, statusId)
            : _sqlHelper.GetInsertCommand(entity, true);
    ...
    public virtual T Insert(T entity, OdbcConnection conn, OdbcTransaction transaction, bool getEntity = false)
    {
        ...
        {
            var command = "VALUES IDENTITY_VAL_LOCAL()";
            var id = conn.Query<long>(command, null, transaction).FirstOrDefault();
            return GetById(id);
        }
    ...
    public virtual T Update(T entity, OdbcConnection conn, OdbcTransaction transaction)
    {
        (string sql, DynamicParameters param) = _sqlHelper.GetUpdateCommand(entity, true);
        conn.Execute(sql, param, transaction);
        return GetById(entity.Id);
    ...
    public virtual IQueryable<T> GetAll(List<Filter> filters)
    {
        ...
        var query = conn.Query<T>(sql, param);
