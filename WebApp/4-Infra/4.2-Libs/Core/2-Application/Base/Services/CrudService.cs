...
public class CrudService<TInput, TOutput, TEntity, TRepository> : BaseService,
...
{
    protected readonly bool _isEfRepository;
    protected readonly TRepository _repository;
    protected readonly IValidator<TInput> _validator;

    public CrudService(AppSettings appSettings,
        ResponseObject responseObject, IValidator<TInput> validator,
        TRepository repository) : base(appSettings, responseObject)
    {
        _validator = validator;
        _repository = repository;
        _isEfRepository = _repository is ICrudEfRepository<TEntity>;
    }
    ...
    public virtual GetAllOutput<TOutput> GetAllPaginated(GetAllInput viewModel)
    {
        ...
        var output = new GetAllOutput<TOutput>
        {
            Rows = _isEfRepository
            ? rows.Skip(viewModel.Skip).Take(viewModel.Take).Adapt<List<TOutput>>()
            : rows.Adapt<List<TOutput>>(),
    ...
    public virtual TOutput Update(TInput viewModel, bool editActiveRows = false)
    {
        UpdateValidate(viewModel, editActiveRows);
    ...
    protected virtual void UpdateValidate(TInput viewModel, bool editActiveRows = false)
    {
        if (viewModel.Id == 0)
        {
    ...
    protected virtual TEntity MapToEntity(TInput viewModel, bool isCreate = true, StatusEnum statusId = StatusEnum.Build)
    {
        var entity = viewModel.Adapt<TEntity>();

        if (entity is IBaseStatusEntity && isCreate)
        {
