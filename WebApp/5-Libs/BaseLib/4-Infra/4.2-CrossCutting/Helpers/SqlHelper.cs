...
public class SqlHelper(IAuthUser usuarioAutenticado)
{
    ...
    public static (string whereSql, DynamicParameters param) GetFilters<T>(List<Filter> filters, bool isOdbc, string orderBy = "", bool isAscending = true) where T : IBaseEntity
    {
        ...
        foreach (var filter in filters)
        {
            ...
            var property = GetProperty<T>(filter);

            if (property == null)
            {
                errors.AppendLine($"A propriedade {filter.Name} não existe, favor entrar em contato com o suporte.");
                continue;
            }
            ...
            if (filter.Name?.ToLower() == nameof(IBaseStatusEntity.StatusId))
            {
                ...
                {
                    whereSql.AppendLine($"AND {columnName} = {paramStr}");
                    param.Add(columnName, status);
                }
                else
                {
            ...
            {
                whereSql.AppendLine($" AND TRIM(UPPER({columnName})) LIKE {paramStr}");
            }
            else if (property.PropertyType == typeof(DateTime) || property.PropertyType == typeof(DateTime?))
            {
                whereSql.AppendLine($" AND DATE({columnName}) = {paramStr}");
            }
            else
            {
                whereSql.AppendLine($" AND {columnName} = {paramStr}");
            ...
            param.Add(columnName, filter.Value);
        }
